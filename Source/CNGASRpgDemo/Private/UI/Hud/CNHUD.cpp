...
{
	//通过上面Overlay的获取方式来一样的获取
	//首先判断Params是否为空
	if (AttributeMenuWidgetController == nullptr)
	{
		//若AttributeMenuWidgetController为空则重新给AttributeMenuWidgetController赋值
		AttributeMenuWidgetController = NewObject<UAttributeMenuWidgetController>(this, AttributeMenuWidgetControllerClass);
		//调用SetWidgetControllerParams方法并进行赋值
		AttributeMenuWidgetController->SetWidgetControllerParams(Params);

		//获取AttributeMenuWidgetController后再进行相应绑定
		AttributeMenuWidgetController->BindCallbackToDepencies();
	}

	

	//返回AttributeMenuWidgetController
